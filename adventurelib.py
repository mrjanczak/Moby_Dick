...
class Exit:
    """Exit from Room(
        ...
        a(A) - area where exit active, 
        point(Point) - position where player starts after comming back"""

    def __init__(self, room = None, a = None, point = None, trans = [0,0], **kwargs):
        self.room = room
        self.a = a
        self.point = point
        for key, value in kwargs.items():
...
class Stairs:
    """Stairs in Room( 
        steps(A) - area where stairs are active, 
...
class A:
    """Polygon area with tag and state
        poly(Polygon)
        active(bool)
        tag(str)"""

    def __init__(self, poly, active = True, tag = None, **kwargs):
        self.poly = poly
        self.active = active
        self.tag = tag
        for key, value in kwargs.items():
...
class Room:
    ...
    def __setattr__(self, name, value):
        ...
        if isinstance(value, Room):
            ...
            object.__setattr__(self, name, Exit(value))
            object.__setattr__(value, reverse, Exit(self))
    ...
    def rev_direction(self, direction):
        """Get reverse exit direction."""
        if direction not in self._directions:
            raise KeyError('%r is not a direction' % direction)
        else: return self._directions[direction]        
...
def no_command_matches(command):
    ...
    say("I don't understand '%s'." % command)
...
def start(help=True, loop=False):
    """Run the game."""
